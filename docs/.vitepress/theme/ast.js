 ...
 *
 * 1. *解析* 将源代码转换为一个更抽象的形式，也就是AST(抽象语法树)，方便后续处理。
 *
 * 2. *转换* 接受解析产生的 AST 并且操纵这些它做任何编译器想让它们做的事。
 *
 * 3. *代码生成* 基于转换后的代码表现形式生成目标代码。
 */
 ...
 *
 * 1. *词法分析* 通过一个叫做tokenizer（token生成器，也叫lexer）的工具将源代码分解成一个个 token。
 *
 *    token是描述编程语言语法的对象。它可以描述数字，标识符，标点符号，运算符等等。
 *
